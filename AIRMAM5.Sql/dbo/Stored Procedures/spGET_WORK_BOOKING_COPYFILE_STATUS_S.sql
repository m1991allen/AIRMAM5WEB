...
-- =============================================
CREATE PROCEDURE [dbo].[spGET_WORK_BOOKING_COPYFILE_STATUS_S]
AS
BEGIN
 	SET NOCOUNT ON;
	...
	ELSE
		...
		FROM
			tblWORK AS WK
		LEFT JOIN
			tbmBOOKING AS BK ON (BK.fnBOOKING_ID = WK.fnGROUP_ID)
		WHERE
			-- 2013/12/10 ERIC ++
			--(WK.fsSTATUS = '_S') AND ((WK.fsTYPE IN ('BOOKING', 'COPYFILE', 'NAS', 'AVID') AND (BK.fsSTATUS LIKE '0%')) OR (WK.fsTYPE = 'TRANSCODE'))
			-- 2013/12/10 ERIC --
			WK.fdSTART_WORK_TIME <= GETDATE() AND (WK.fsSTATUS = '_S') AND (WK.fsTYPE IN ('BOOKING', 'COPYFILE', 'NAS', 'AVID')) AND (BK.fsSTATUS LIKE '0%')
		ORDER BY
