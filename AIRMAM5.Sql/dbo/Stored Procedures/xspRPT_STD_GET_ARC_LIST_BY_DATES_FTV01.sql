...
-- Description:	<FTV_USE 取得某段日期內節目製作單位的各節目上傳的V/A/P/D統計>
-- 記錄:	<2014/04/15><Albert.Chen><調整欄位fsDESCRIPTION長度為NVARCHAR(MAX)>
-- =============================================
CREATE PROCEDURE [dbo].[xspRPT_STD_GET_ARC_LIST_BY_DATES_FTV01]
...
BEGIN
	...
	SET NOCOUNT ON;
		...
		DECLARE	@tblRESULT TABLE( LV0_NAME NVARCHAR(50), LV1_NAME NVARCHAR(50), LV2_NAME NVARCHAR(50), LV3_NAME NVARCHAR(50), LV4_NAME NVARCHAR(50), 
								...
								LV5_ID INT, LV6_ID INT, LV7_ID INT, LV8_ID INT, LV9_ID INT, _DIRTYPE VARCHAR(1), _DIRID BIGINT, _DIRPID BIGINT,  _SUBJLIST NVARCHAR(MAX), _DIRNAME NVARCHAR(50), _DESC NVARCHAR(MAX), V_CNT INT, A_CNT INT, P_CNT INT, D_CNT INT)

		DECLARE @tblQUEUE TABLE (ID INT, DIR_ID INT, _LIST VARCHAR(50), _DIRTYPE VARCHAR(1), _DIRID BIGINT, _DIRPID BIGINT, _DIRNAME NVARCHAR(50), _DESC NVARCHAR(MAX)) 

		-----

		INSERT @tblQUEUE 
		SELECT ROW_NUMBER() OVER(ORDER BY fnDIR_ID), fnDIR_ID, dbo.fnGET_DIR_PARENT_LIST(fnDIR_ID), fsDIRTYPE, fnDIR_ID, fnPARENT_ID, fsNAME, fsDESCRIPTION
		...
		SELECT
				...
				_DIRTYPE, _DIRID, _DIRPID,  '', _DIRNAME, _DESC,dbo.fnGET_MEDIA_CNT_BY_DATES_AND_TYPE_AND_DIRID('V',_DIRID,@SDATE,@EDATE),dbo.fnGET_MEDIA_CNT_BY_DATES_AND_TYPE_AND_DIRID('A',_DIRID,@SDATE,@EDATE),dbo.fnGET_MEDIA_CNT_BY_DATES_AND_TYPE_AND_DIRID('P',_DIRID,@SDATE,@EDATE),dbo.fnGET_MEDIA_CNT_BY_DATES_AND_TYPE_AND_DIRID('D',_DIRID,@SDATE,@EDATE)
		FROM	@tblQUEUE AS Q
		...
		SELECT LV1_NAME, LV2_NAME, LV3_NAME, LV4_NAME, IIF(LV4_NAME IS NULL,IIF(LV3_NAME IS NULL,'    ' + _DIRNAME,'        '+ _DIRNAME),'            '+ _DIRNAME) AAA
				, _DIRTYPE, _DIRID, _DESC, V_CNT, A_CNT, P_CNT, D_CNT
		FROM @tblRESULT 
		WHERE LV1_ID = 1000 AND _DIRID <> 1000
		ORDER BY 
			LV0_ORDER, LV0_NAME, 
			LV1_ORDER, LV1_NAME, 
			LV2_ORDER, LV2_NAME, 
