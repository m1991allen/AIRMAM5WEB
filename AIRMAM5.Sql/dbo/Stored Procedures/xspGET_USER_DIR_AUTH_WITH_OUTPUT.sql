...
-- =============================================
-- 描述:	取出使用者有權限的DIR(給登入時spGET_USERS_BY_PASSWORD呼叫用)
-- 記錄:	<2019/07/27><David.Sin><新增本預存>
-- =============================================
CREATE PROCEDURE [dbo].[xspGET_USER_DIR_AUTH_WITH_OUTPUT]
...
BEGIN
	...
	BEGIN TRY
		...
		UPDATE #tbResult 
		SET _USER = 'Y'
		WHERE fnDIR_ID IN (SELECT [fnDIR_ID] FROM [dbo].[tbmDIR_GROUP] WHERE [fnGROUP_ID] IN (SELECT [fsGROUP_ID] FROM [dbo].[tbmUSER_GROUP] WHERE [fsUSER_ID] = @fsUSER_ID))
