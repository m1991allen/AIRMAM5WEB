...
-- =============================================
-- 描述:	取出 ARC_AUDIO 自訂欄位資料
-- 記錄:	<2011/11/08><Eric.Huang><新增本預存>
...
-- =============================================
CREATE PROCEDURE [dbo].[spGET_ARC_AUDIO_ATTRIBUTE]
...
BEGIN
	...
	DECLARE @fsSUBJECT_ID VARCHAR(12) = (	SELECT fsSUBJECT_ID 
											FROM tbmARC_AUDIO								--<<TODO:影音圖文
											WHERE (fsFILE_NO = @fsFILE_NO)	)

	/*取出此主題檔所屬的目錄中, 此媒體檔套用的樣板編號*/
	DECLARE @TEMP_ID INT = (SELECT fnTEMP_ID_AUDIO											--<<TODO:影音圖文
							FROM tbmDIRECTORIES
		...
		WHILE (@@FETCH_STATUS=0)
			...
			---------開始處理每一筆資料
				...
				SET @SQL = 'SELECT @value = ' + @fsFIELD + 
							' FROM tbmARC_AUDIO WHERE (fsFILE_NO = ''' + @fsFILE_NO + ''')'	--<<TODO:影音圖文
