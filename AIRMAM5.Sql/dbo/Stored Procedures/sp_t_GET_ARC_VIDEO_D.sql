...
-- =============================================
-- 描述:	取出暫時刪除的-影片段落描述檔 資料
-- 記錄:	<2019/09/17><David.Sin><新增本預存>
-- =============================================
CREATE PROCEDURE [dbo].[sp_t_GET_ARC_VIDEO_D]
	@fnINDEX_ID		BIGINT
AS
BEGIN
 	SET NOCOUNT ON;
	...
	SELECT 
			[dbo].[t_tbmARC_VIDEO_D].[fsFILE_NO],
		...
		FROM
			[dbo].[t_tbmARC_VIDEO_D]
				JOIN [dbo].[t_tbmARC_INDEX] IDX ON [dbo].[t_tbmARC_VIDEO_D].[fsFILE_NO] = IDX.fsFILE_NO
				LEFT JOIN [dbo].[tbmUSERS] USERS_CRT ON [dbo].[t_tbmARC_VIDEO_D].[fsCREATED_BY] = USERS_CRT.fsLOGIN_ID
				LEFT JOIN [dbo].[tbmUSERS] USERS_UPD ON [dbo].[t_tbmARC_VIDEO_D].[fsUPDATED_BY] = USERS_UPD.fsLOGIN_ID
		WHERE
