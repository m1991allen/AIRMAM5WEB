...
-- =============================================
-- 描述:	取出全部 DIRECTORIES主檔資料&其主題檔&VIDEO
-- 記錄:	<2015/08/11><Eric.Huang><新增本預存> copy from spGET_DIRECTORIES_AND_SUBJECTS_BY_CFG_LEVEL
-- 記錄:	<2015/08/11><Eric.Huang><修改本預存> 新增 ARC_DOC 的欄位
-- =============================================
CREATE PROCEDURE [dbo].[xspGET_DIRECTORIES_AND_SUBJECTS_AND_VIDEOS_BY_CFG_LEVEL]
...
BEGIN
	...
	SELECT	[fnDIR_ID],			[fsNAME],			[fnPARENT_ID],		[fsDESCRIPTION],		[fsDIRTYPE],
			...
			[fdCREATED_DATE],	[fsCREATED_BY],		[fdUPDATED_DATE],	[fsUPDATED_BY],
			 _index = 1, _level = 0	
	--FROM tbmDIRECTORIES WHERE (([fnDIR_ID] = 571) or ([fnDIR_ID] = 202))
		...
		IF (@USER = 'FTV')
			BEGIN
				...
				SELECT
					...
					SUBJ.fsSUBJ_ID fsSUBJ_ID, SUBJ.fsTITLE,  SUBJ.fsDESCRIPTION, SUBJ.fsGroups,
						...
						,VDO.fsFILE_NO VDO_FILE_NO
						,VDO.fsTITLE VDO_TITLE
						,VDO.fsDESCRIPTION VDO_DESC

						-- 2015/11/03 ERIC 新增
						,DOC.fsFILE_NO DOC_FILE_NO
						,DOC.fsTITLE DOC_TITLE
						,DOC.fsDESCRIPTION DOC_DESC
						--,PHO.fsFILE_NO PHO_FILE_NO
						--,PHO.fsTITLE PHO_TITLE
						--,PHO.fsDESCRIPTION PHO_DESC
				...
				LEFT JOIN tbmARC_VIDEO AS VDO ON fsSUBJ_ID = VDO.fsSUBJECT_ID
				LEFT JOIN tbmARC_DOC AS DOC ON fsSUBJ_ID = DOC.fsSUBJECT_ID
				--LEFT JOIN tbmARC_PHOTO AS PHO ON fsSUBJ_ID = PHO.fsSUBJECT_ID
