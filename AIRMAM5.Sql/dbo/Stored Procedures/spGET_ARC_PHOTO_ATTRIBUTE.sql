...
-- =============================================
CREATE PROCEDURE [dbo].[spGET_ARC_PHOTO_ATTRIBUTE]
...
BEGIN
	...
	DECLARE @TEMP_ID INT = (SELECT fnTEMP_ID_PHOTO
							FROM tbmDIRECTORIES
		...
		WHILE (@@FETCH_STATUS=0)
			...
			---------開始處理每一筆資料
				...
				SET @SQL = 'SELECT @value = ' + @fsFIELD + 
							' FROM tbmARC_PHOTO WHERE (fsFILE_NO = ''' + @fsFILE_NO + ''')'
