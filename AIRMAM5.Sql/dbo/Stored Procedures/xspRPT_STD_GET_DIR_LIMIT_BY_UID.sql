...
-- =============================================
CREATE PROCEDURE [dbo].[xspRPT_STD_GET_DIR_LIMIT_BY_UID]
...
BEGIN
	...
	BEGIN TRY
		--DECLARE @LOGIN_ID NVARCHAR(50) =  'eric.huang'-- 'terry.kao'

		---

		DECLARE @fnUSER_ID INT = ISNULL((SELECT TOP 1 [fnUSER_ID] FROM [dbo].[tbmUSERS] WHERE ([fsLOGIN_ID] = @LOGIN_ID)), -1)
		...
		ELSE
			BEGIN
				...
				DECLARE	@tblRESULT TABLE( LV0_NAME NVARCHAR(50), LV1_NAME NVARCHAR(50), LV2_NAME NVARCHAR(50), LV3_NAME NVARCHAR(50), LV4_NAME NVARCHAR(50), 
										...
										_管理 VARCHAR(5), _檢視 VARCHAR(5), _主題 VARCHAR(5), _影片 VARCHAR(5), _聲音 VARCHAR(5), _圖片 VARCHAR(5), _文件 VARCHAR(5), _顯示 VARCHAR(5))
				...
				WHERE	(dbo.fnGET_STRING_MATCH(
						...
						(dbo.fnGET_STRING_MATCH(
							(SELECT TOP 1 [fsADMIN_USER] FROM [dbo].[tbmDIRECTORIES] WHERE ([fnDIR_ID] = _DIRID)), (@LOGIN_ID + ';')
						) = 'Y')
				...
				/*檢查所有目錄 若帳號或群組有檢視或編輯權限 => 「檢視」欄標記為 "V"
															   其他「_」開頭欄標記權限VIUD*/
				UPDATE	@tblRESULT
				SET		_主題 = dbo.fnGET_USER_DIR_LIMIT(@LOGIN_ID, _DIRID, 'S'),
						_影片 = dbo.fnGET_USER_DIR_LIMIT(@LOGIN_ID, _DIRID, 'V'),
						_聲音 = dbo.fnGET_USER_DIR_LIMIT(@LOGIN_ID, _DIRID, 'A'),
						_圖片 = dbo.fnGET_USER_DIR_LIMIT(@LOGIN_ID, _DIRID, 'P'),
						_文件 = dbo.fnGET_USER_DIR_LIMIT(@LOGIN_ID, _DIRID, 'D')
