...
-- 記錄:	<2013/09/17><Eric.Huang><修改本預存新增 @FILE_SPACE_NAME> 
-- 記錄:	<2013/12/10><Eric.Huang><因應EBC需要加入流程:DAILY重轉時，採用TRANSCODE流程，故將TYPE為TRANSCODE加入到TMS流程中>
-- =============================================
CREATE PROCEDURE [dbo].[spGET_WORK_BOOKING_COPYFILE_STATUS_U]
AS
BEGIN
 	SET NOCOUNT ON;
	...
	ELSE
		...
		SELECT
			...
			--_SM_FILE_SPACE_NAME = '/HVIDEO',
			_SM_FILE_SPACE_NAME = @FILE_SPACE_NAME,

			--_SM_FILE_NAME  = (dbo.fnGET_TSM_PATH_BY_FILE_NO_EBC((select dbo.fnGET_WORK_PARAMETERS_ANALYZE_BOOKING(fsPARAMETERS,1,'N','N'))))
			_SM_FILE_NAME  = @FILE_RESTORE_SPACE + (dbo.fnGET_TSM_PATH_BY_FILE_NO((select dbo.fnGET_WORK_PARAMETERS_ANALYZE_BOOKING(fsPARAMETERS,1,'N','N'))))
		FROM
			tblWORK AS WK
				LEFT JOIN tbmBOOKING AS BK ON (BK.fnBOOKING_ID = WK.fnGROUP_ID)
		WHERE
			-- 2013/12/10 ERIC ++
			--(WK.fsSTATUS = '_U') AND ((WK.fsTYPE IN ('BOOKING', 'COPYFILE', 'NAS', 'AVID') AND (BK.fsSTATUS LIKE '0%')) OR (WK.fsTYPE = 'TRANSCODE'))
			-- 2013/12/10 ERIC --
			WK.fdSTART_WORK_TIME <= GETDATE() AND (WK.fsSTATUS = '_U') AND (WK.fsTYPE IN ('BOOKING', 'COPYFILE', 'NAS', 'AVID')) AND (BK.fsSTATUS LIKE '0%')
		ORDER BY
