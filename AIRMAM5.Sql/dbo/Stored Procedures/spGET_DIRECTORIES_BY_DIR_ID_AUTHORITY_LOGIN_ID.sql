...
-- =============================================
-- 描述:	依照DIR_ID&LOGIN ID取出該使用者針對該目錄有哪些權限
-- 記錄:	<2016/09/19><David.Sin><新增本預存>
-- =============================================
CREATE PROCEDURE [dbo].[spGET_DIRECTORIES_BY_DIR_ID_AUTHORITY_LOGIN_ID]
...
BEGIN
	...
	INSERT
		#tbDIR_GROUP_USER
	---以下是把有設定在[DIRECTORIES]目錄fsADMIN_GROUP的管理群組加入
	...
	---以下是把有設定在[tbmDIR_GROUP]目錄的使用群組加入
		UNION
	...
	---以下是把有設定在[DIRECTORIES]目錄fsADMIN_USER的管理使用者加入
		UNION		
	...
	---以下是把有設定在[tbmDIR_USER]目錄的使用者加入
		UNION
	SELECT	'U', '', '', U.fsUSER_ID, U.fsLOGIN_ID, U.fsNAME, '', '',	'', '', '',	'', '', 0
	FROM	dbo.tbmUSERS AS U JOIN tbmDIR_USER AS DU ON (DU.fsLOGIN_ID = U.fsUSER_ID)
	...
	WHERE
		...
		EXISTS(SELECT fnDIR_ID FROM #tbDIR_GROUP_USER AS G JOIN #tbResult B ON B.fsADMIN_GROUP LIKE '%'+ G.GROUP_ID+';%' AND #tbDIR_GROUP_USER.GROUP_ID = G.GROUP_ID)
