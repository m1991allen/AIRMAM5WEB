...
-- =============================================
-- 描述:	取出 暫時刪除的聲音 自訂欄位資料
-- 記錄:	<2019/09/17><David.Sin><新增本預存>
-- =============================================
CREATE PROCEDURE [dbo].[sp_t_GET_ARC_AUDIO_ATTRIBUTE]
...
BEGIN
	...
	/*取出此媒體檔所屬的主題檔*/
	DECLARE @fsSUBJECT_ID VARCHAR(12) = (	SELECT [dbo].[t_tbmARC_AUDIO].[fsSUBJECT_ID] 
											FROM [dbo].[t_tbmARC_AUDIO]								--<<TODO:影音圖文
											WHERE ([dbo].[t_tbmARC_AUDIO].[fsFILE_NO] = @fsFILE_NO)	)

	/*取出此主題檔所屬的目錄中, 此媒體檔套用的樣板編號*/
	DECLARE @TEMP_ID INT = (SELECT [dbo].[tbmDIRECTORIES].[fnTEMP_ID_AUDIO]											--<<TODO:影音圖文
	...
	OPEN CSR1
		...
		WHILE (@@FETCH_STATUS=0)
			...
			---------開始處理每一筆資料
				...
				SET @SQL = 'SELECT @value = ' + @fsFIELD + 
							' FROM t_tbmARC_AUDIO WHERE (fsFILE_NO = ''' + @fsFILE_NO + ''')'	--<<TODO:影音圖文
		...
		SELECT * FROM @tbResult ORDER BY fnORDER
END
