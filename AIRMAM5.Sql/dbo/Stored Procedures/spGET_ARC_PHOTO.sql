...
-- =============================================
CREATE PROCEDURE [dbo].[spGET_ARC_PHOTO]
...
BEGIN
	...
	SELECT 
		...
		_sFILE_URL_L = @fsMEDIA_PREVIEW_URL + 'P/' + REPLACE(REPLACE(tbmARC_PHOTO.fsFILE_PATH_L,(SELECT fsVALUE FROM tbzCONFIG WHERE fsKEY = 'MEDIA_FOLDER_P'),''),'\','/') +
							+ [fsFILE_NO] + '_L.' + fsFILE_TYPE
	FROM
		tbmARC_PHOTO 
			JOIN tbmSUBJECT ON tbmARC_PHOTO.fsSUBJECT_ID = tbmSUBJECT.fsSUBJ_ID
			JOIN reADMIN A ON tbmSUBJECT.fnDIR_ID = A.fnDIR_ID
			LEFT JOIN tbmUSERS USERS_CRT ON tbmARC_PHOTO.fsCREATED_BY = USERS_CRT.fsLOGIN_ID
			LEFT JOIN tbmUSERS USERS_UPD ON tbmARC_PHOTO.fsUPDATED_BY = USERS_UPD.fsLOGIN_ID
