...
-- =============================================
-- 描述:	取出 暫時刪除的-圖片 自訂欄位資料
-- 記錄:	<2019/09/17><David.Sin><新增本預存>
-- =============================================
CREATE PROCEDURE [dbo].[sp_t_GET_ARC_PHOTO_ATTRIBUTE]
...
BEGIN
	...
	/*取出此媒體檔所屬的主題檔*/
	DECLARE @fsSUBJECT_ID VARCHAR(12) = (	SELECT [dbo].[t_tbmARC_PHOTO].[fsSUBJECT_ID] 
											FROM [dbo].[t_tbmARC_PHOTO]
											WHERE ([dbo].[t_tbmARC_PHOTO].[fsFILE_NO] = @fsFILE_NO)	)

	/*取出此主題檔所屬的目錄中, 此媒體檔套用的樣板編號*/
	DECLARE @TEMP_ID INT = (SELECT [dbo].[tbmDIRECTORIES].[fnTEMP_ID_PHOTO]
		...
		WHILE (@@FETCH_STATUS=0)
			...
			---------開始處理每一筆資料
				...
				SET @SQL = 'SELECT @value = ' + @fsFIELD + 
							' FROM t_tbmARC_PHOTO WHERE (fsFILE_NO = ''' + @fsFILE_NO + ''')'
