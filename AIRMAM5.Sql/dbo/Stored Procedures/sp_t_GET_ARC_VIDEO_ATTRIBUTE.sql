...
-- =============================================
-- 描述:	取出 暫時刪除的-影片 自訂欄位資料
-- 記錄:	<2019/09/17><David.Sin><新增本預存>
-- =============================================
CREATE PROCEDURE [dbo].[sp_t_GET_ARC_VIDEO_ATTRIBUTE]
...
BEGIN
	...
	DECLARE @tbResult TABLE(fsFIELD			VARCHAR(50),
		...
		/*取出此媒體檔所屬的主題檔*/
		DECLARE @fsSUBJECT_ID VARCHAR(12) = (	SELECT [dbo].[t_tbmARC_VIDEO].[fsSUBJECT_ID] 
												FROM [dbo].[t_tbmARC_VIDEO]
												WHERE ([dbo].[t_tbmARC_VIDEO].[fsFILE_NO] = @fsFILE_NO)	)

		/*取出此主題檔所屬的目錄中, 此媒體檔套用的樣板編號*/
		DECLARE @TEMP_ID INT = (SELECT [dbo].[tbmDIRECTORIES].[fnTEMP_ID_VIDEO]
		...
		INSERT
			@tbResult	
		SELECT
			[dbo].[tbmTEMPLATE_FIELDS].[fsFIELD], [dbo].[tbmTEMPLATE_FIELDS].[fsFIELD_NAME], [dbo].[tbmTEMPLATE_FIELDS].[fsFIELD_TYPE], [dbo].[tbmTEMPLATE_FIELDS].[fnORDER], [dbo].[tbmTEMPLATE_FIELDS].[fsCODE_ID], [dbo].[tbmTEMPLATE_FIELDS].[fsCODE_CTRL], '', [dbo].[tbmTEMPLATE_FIELDS].[fsISNULLABLE],[dbo].[tbmTEMPLATE_FIELDS].[fnCODE_CNT]
		FROM
